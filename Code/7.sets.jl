...
# it to accept elements of type Any, which can be a minor slowdown
s = Set()

# This is of course fixed by inferring the type when creating the dict
s = Set{Int}()
push!(s, 2)
display(s)
...
@show s1 ∪ s2 # Equivalent to the line above (\cup for the unicode macro)

@show intersect(s1, s2) # Set intersect
@show s1 ∩ s2 # Unicode version (\cap)

@show setdiff(s1, s2) # Set difference. No unicode replacement for this one
@show symdiff(s1, s2) # symmetric difference. No unicode here either

union!(s1, s2) # Equivalent to s1 = s1 ∪ s2
@show s1

intersect!(s1, s2) # Equivalent to s1 = s1 ∩ s2
@show s1
